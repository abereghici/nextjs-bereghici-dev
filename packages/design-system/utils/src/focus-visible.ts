...
function focusTriggersKeyboardModality(node: Element) {
  ...
  const { tagName, isContentEditable } = node;

  if (tagName === 'INPUT') {
    const { type, readOnly } = node as HTMLInputElement;
    if (inputTypesWhitelist.includes(type) && !readOnly) {
      return true;
    }
  }

  if (tagName === 'TEXTAREA') {
    const { readOnly } = node as HTMLInputElement;
    if (!readOnly) {
      return true;
    }
  }
...
export function isFocusVisible(event: React.FocusEvent) {
  try {
...
export function initFocusVisible(node?: HTMLElement) {
...
export const forkFocus =
  (
    ...
    handler: (event: React.FocusEvent<never>) => void
  ) =>
