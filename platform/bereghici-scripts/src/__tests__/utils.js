...
test('resolveBrgScripts resolves to src/index.js when in the bereghici-scripts package', () => {
  mockPkg({ package: { name: '@bereghici/bereghici-scripts' } });
  expect(require('../utils').resolveBrgScripts()).toBe(
    require.resolve('../').replace(process.cwd(), '.')
  );
});

test('resolveBrgScripts resolves to bereghici-scripts if not in the bereghici-scripts package', () => {
  mockPkg({ package: { name: '@bereghici/not-bereghici-scripts' } });
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'));
  expect(require('../utils').resolveBrgScripts()).toBe('bereghici-scripts');
});
