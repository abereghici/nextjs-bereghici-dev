...
date: '2021-08-16'
image: /static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part1.webp
---

<Image
  alt={`Rushjs logo`}
  src={`/static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part1.webp`}
  blurDataURL={`/static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part1.webp`}
  width={800}
  height={400}
  priority
  placeholder="blur"
...
I assume you already created an empty Github repository for this project.
Let's clone it locally and let the magic begin!
...
You don't really want to perform a migration to monorepo if you lose all the history
of your projects. If everything will point to the commit where you merged the projects,
you won't be able to revert to the previous commits, or run `git blame` or `git bisect`.

We can copy all projects inside of the monorepo and keep the git history of each project
with a single git command: `git subtree`.
...
The last step is to register `@monorepo/react-app` project in rush configuration file.
...
This tells to Rush that it should manage this project. Next, run `rush update` to install
the dependencies of `react-app` project. This command can be launched in any subfolder of
the repo folder that contains `rush.json` file.
...
We want to have consistent syntax and formatting across all code files in our monorepo.
So we'll apply Prettier globally for the entire repository. We'll run it during `git commit`.
...
You also need to make a `.prettierignore` file to tell Prettier which files to skip.
It is recommended for `.prettierignore` to extend the same patterns used in `.gitignore`.

```bash
cp .gitignore .prettierignore
```

Once the configuration is set up, next we need to invoke Prettier manually
to reformat all the existing source files.
...
To speed up the prettier process on `git commit` hook, we'll use `prettier-quick`
...
Next, we will create a rush prettier custom command that invokes
the `pretty-quick` tool. Add this to the "commands" section of `config/rush/command-line.json` file:
...
The last step is to add a Git hook that invokes rush prettier automatically whenever
...
Let's go to the [next part](/blog/build-a-scalable-front-end-with-rush-monorepo-and-react--webpack+jest)
