...
date: '2021-08-18'
image: /static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part3.webp
---

<Image
  alt={`Rushjs logo`}
  src={`/static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part3.webp`}
  blurDataURL={`/static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part3.webp`}
  width={800}
  height={400}
  priority
  placeholder="blur"
...
ESLint is a dominant tool for linting TypeScript and JavaScript code. We'll use it
together with Lint Staged to achieve the goal "_Enforced rules for code quality_" we defined in
[Part 1](/blog/build-a-scalable-front-end-with-rush-monorepo-and-react--repo-setup+import-projects+prettier).

ESLint works with a set of rules you define. If you already have a configuration for ESLint
that you like, you can add it in our next setup. We'll be using AirBnB’s ESLint config,
which is the most common rules list for JavaScript projects. As of mid-2021,
it gets over 2.7 million downloads per week from NPM.
...
We'll split ESLint rules in multiple files. In `base.js` file we'll define the main rules
that can be applied to all packages. In `react.js` will be the React-specific rules.
...
Here we're extending `airbnb` and `prettier` configurations. Here you can include
...
We have to provide a `react` version to `react-app` configuration. Instead of hardcoding it
we'll use `read-pkg-up` to get the version from `package.json`. `semver` is used to help us
...
create a `lint` script that runs ESLint bin for you. I'm feeling more comfortable
...
The most important function here is `resolveBin` that will try to resolve the binary
for a given module.
...
Run `rush update` following by `rushx lint`. At this point you should see a bunch of
ESLint errors. As an exercise, you can try to fix them by enabling / disabling some rules
in `eslint-config` or modify `react-app` project to make it pass the linting.

#### Add lint-staged command to react-scripts

We'll follow the same approach as we did with `lint` script. Create `lint-staged.js` file inside
of `packages/react-scripts/scripts` and add the following:
...
Next step is to register a `lint-staged` rush command in `common/config/command-line.json`,
as we did with `prettier` command in [Part 1](/blog/build-a-scalable-front-end-with-rush-monorepo-and-react--repo-setup+import-projects+prettier).
...
Now, let's run `lint-staged` command on git `pre-commit` hook.
Open `common/git-hooks/pre-commit` and add the append to the end of the file:
...
If you encountered any issues during the process, you can see the code related to this post
[here](https://github.com/abereghici/rush-monorepo-boilerplate/tree/049506a98529d4931b2a2ecb629957e0d93e8a8d).

Let's go to the [next part](/blog/build-a-scalable-front-end-with-rush-monorepo-and-react--github-actions+netlify)
where we'll see how to use Github Actions with Netlify to automate our deployment workflow.
