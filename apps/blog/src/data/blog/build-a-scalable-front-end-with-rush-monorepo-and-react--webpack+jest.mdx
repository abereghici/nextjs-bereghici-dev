...
date: '2021-08-17'
image: /static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part2.webp
---

<Image
  alt={`Rushjs logo`}
  src={`/static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part2.webp`}
  blurDataURL={`/static/images/build-a-scalable-front-end-with-rush-monorepo-and-react-part2.webp`}
  width={800}
  height={400}
  priority
  placeholder="blur"
...
Create a `package.json` file inside the `react-scripts` folder and add the following configuration:
...
Move `config` and `scripts` folders from `react-app` to `react-scripts` and let's
adjust the configurations.

Remove `ESLint` plugin and its dependencies from `packages/react-scripts/config/webpack.config.js`.
...
The most important change in this file is the `resolveApp` function call.
We shouldn't use relative paths anymore, because we are moving the
configuration files to `react-scripts` package.
We'll get the project files paths using a combination between `path.resolve` and `process.cwd()`.
...
This will be the entry-point of our CLI. Every time when you'll want to add a new command
you have to adjust this file.

Create another folder named `lib` and in `react-app.d.ts` file add the following code:
...
Here we'll store all type definitions for css, sass, scss and image files and we'll reuse
in all react applications inside of the monorepo. You can address to these
type definitions by creating a file in your react project named `react-app-env.d.ts`
...
Now, let's cleanup `apps/react-app/package-json` file by removing all dependencies
related to build tools. We'll also remove the configurations for `eslint`, `babel` or `jest`,
because we'll store them in a separate files.
In our dependencies we'll list `@monorepo/react-scripts` package.
...
Run `rush update` to install all dependencies and let's try to build our app with
the `react-script` package we just created.
...
Open `package.json` from `@monorepo/react-scripts` and add the following configuration:
...
Modify the `checkBrowsers` function call from
`packages/react-scripts/build.js` and `packages/react-scripts/start.js` with:
...
Our current `test` script expects to find a `jest` configuration in each project.
We'll modify it to use a shared configuration.

Create a `utils` folder in `packages/react-scripts/scripts` and in `createJestConfig.js`
...
Now let's use this configuration. Modify the `test` script in `packages/react-scripts/scripts/test.js`
to look like this:
...
Now we can run `rushx test` inside of `react-app` project and test the shared configuration.

If you encountered any issues during the process, you can check all changes
in [this commit](https://github.com/abereghici/rush-monorepo-boilerplate/commit/de30f0a1bc1f3958e34bcccf7089d3439d153e2c).

In the [next part](/blog/build-a-scalable-front-end-with-rush-monorepo-and-react--eslint+lint-staged)
